...
public class Zone {
    ...
    public boolean isZoneDaiHoiVoThuat;
    ...
    public void pickItem2(Player player, int itemMapId) {
        ...
        if (itemMap != null) {
            if (itemMap.playerId == player.id || itemMap.playerId == -1) {
                ...
                switch (itemMap.itemTemplate.type) {
                    case 9: // vàng
                        if (player.inventory.gold + itemMap.quantity <= Inventory.LIMIT_GOLD) {
                            player.inventory.gold += itemMap.quantity;
                        ...
                        break;
                    case 10: // ngọc xanh
                        player.inventory.gem += itemMap.quantity;
                        Service.getInstance().sendMoney(player);
                        picked = true;
                        break;
                    case 34: // hồng ngọc
                        player.inventory.ruby += itemMap.quantity;
                        Service.getInstance().sendMoney(player);
                        picked = true;
                        break;
                }
...
//            }
    ...
    public void mapInfo(Player pl) {
        ...
        try {
            ...
            // msg.writer().writeShort(0);
            try {
                byte[] bgItem = FileIO.readFile("data/nrolove/map/item_bg_map_data/" + this.map.mapId);
            ...
            // msg.writer().writeShort(0);
            try {
                byte[] effItem = FileIO.readFile("data/nrolove/map/eff_map/" + this.map.mapId);
