...
public class EffectSkin {
    ...
    public void update() {
        updateVoHinh();
        if (this.player.zone != null && !this.player.zone.map.isMapOffline) {
            updateOdo();
            updateXenHutXungQuanh();
        }
        if (!this.player.isBoss && !this.player.isPet && !this.player.isNewMiniPet) {
            updateTrainArmor();
        }
    ...
    private void updateCTHaiTac() {
        if (this.player.setClothes.ctHaiTac != -1
            ...
            for (Player pl : players) {
                ...
                if (!pl.isNewMiniPet &&!pl.isPet && Util.canDoWithTime(lastTimeUpdateCTHT, 5000)) {
