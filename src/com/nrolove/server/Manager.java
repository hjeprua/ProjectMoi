...
import com.nrolove.jdbc.daos.ShopDAO;
...
import com.nrolove.models.task.SideTaskTemplate;
import com.nrolove.models.task.SubTaskMain;
...
public class Manager {
    ...
    public static int MAX_PLAYER = 1500;
    ...
    public static final List<com.nrolove.models.map.Map> MAPS = new ArrayList<>();
    ...
    private void initMap() {
        ...
        for (MapTemplate mapTemp : MAP_TEMPLATES) {
            ...
            com.nrolove.models.map.Map map = new com.nrolove.models.map.Map(mapTemp.id,
    ...
    public static void loadPart() {
        ...
        try (Connection con = DBService.gI().getConnection();) {
            ...
            DataOutputStream dos = new DataOutputStream(new FileOutputStream("data/nrolove/update_data/part"));
...
//            }
//            for (int i = 0; i < 4; i++) {
//                dos = new DataOutputStream(new FileOutputStream("data/nrolove/data_img_version/x" + (i + 1) + "/img_version"));
...
//            }
//            Logger.success("Load cải trang thành công (" + CAI_TRANGS.size() + ")\n");
            ...
            Logger.success("Load item option template thành công (" + ITEM_OPTION_TEMPLATES.size() + ")\n");
            ...
            File folder = new File("data/nrolove/data_lucky_round_reward");
            ...
            //load reward mob
            folder = new File("data/nrolove/data_mob_reward");
            for (File fileEntry : folder.listFiles()) {
                if (!fileEntry.isDirectory()) {
                    BufferedReader br = new BufferedReader(new FileReader(fileEntry));
                    String line = null;
...
//                                    mobReward.events.add(itemReward);
            ...
            Logger.success("Load reward lucky round thành công (" + MOB_REWARDS.size() + ")\n");
    ...
    public void loadProperties() throws IOException {
        ...
        properties.load(new FileInputStream("data/nrolove/nrolove.properties"));
        Object value = null;
        //###Config db
        if ((value = properties.get("server.nrolove.db.driver")) != null) {
            DBService.DRIVER = String.valueOf(value);
        }
        if ((value = properties.get("server.nrolove.db.ip")) != null) {
            DBService.DB_HOST = String.valueOf(value);
        }
        if ((value = properties.get("server.nrolove.db.port")) != null) {
            DBService.DB_PORT = Integer.parseInt(String.valueOf(value));
        }
        if ((value = properties.get("server.nrolove.db.name")) != null) {
            DBService.DB_NAME = String.valueOf(value);
        }
        if ((value = properties.get("server.nrolove.db.us")) != null) {
            DBService.DB_USER = String.valueOf(value);
        }
        if ((value = properties.get("server.nrolove.db.pw")) != null) {
            DBService.DB_PASSWORD = String.valueOf(value);
        }
        if ((value = properties.get("server.nrolove.db.max")) != null) {
            DBService.MAX_CONN = Integer.parseInt(String.valueOf(value));
        }

        //###Config sv
        if ((value = properties.get("server.nrolove.port")) != null) {
            ServerManager.PORT = Integer.parseInt(String.valueOf(value));
        }
        if ((value = properties.get("server.nrolove.name")) != null) {
            ServerManager.NAME = String.valueOf(value);
        }
        if ((value = properties.get("server.nrolove.sv")) != null) {
            SERVER = Byte.parseByte(String.valueOf(value));
        }
        String linkServer = "";
        for (int i = 1; i <= 10; i++) {
            value = properties.get("server.nrolove.sv" + i);
            if (value != null) {
                linkServer += String.valueOf(value) + ":0,";
            }
        }
        DataGame.LINK_IP_PORT = linkServer.substring(0, linkServer.length() - 1);
        if ((value = properties.get("server.nrolove.waitlogin")) != null) {
            SECOND_WAIT_LOGIN = Byte.parseByte(String.valueOf(value));
        }
        if ((value = properties.get("server.nrolove.maxperip")) != null) {
            MAX_PER_IP = Byte.parseByte(String.valueOf(value));
        }
        if ((value = properties.get("server.nrolove.maxplayer")) != null) {
            MAX_PLAYER = Integer.parseInt(String.valueOf(value));
        }
        if ((value = properties.get("server.nrolove.expserver")) != null) {
